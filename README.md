...
- **get-caller-identity output.PNG** → Verified AWS account identity.  
- **Clean credentials file.PNG** → Correctly configured access keys.  
- **Broken credentials file.PNG** → Example of misconfigured credentials (troubleshooting proof).  
- **IAM console showing Active key.PNG** → Console view confirming active key status.

---

## 2. IAM Role & Policy
- **policy.PNG** → Attached IAM policy JSON.  
- **iam-role-armPN.PNG** → Role ARN for `lambda-basic-execution-r014`.  
- **Trusted entity type.PNG** → Trust relationship showing `lambda.amazonaws.com` as the trusted entity.

---

## 3. Lambda Deployment
- **lambda-file.png** → Python handler code (`lambda_function.py`).  
- **lambda-zip-update.png** → Updated ZIP archive containing the handler.  
- **Lambda-CreateFunction-AccessKe.png** → CLI run for function creation.  
- **lambda-createPNG.PNG** → Successful Lambda creation output.  
- **Updated JSON.PNG** → Corrected JSON output from CLI.  
- **Lambda function successfully PNG.PNG** → Console confirmation of active Lambda function.

---

## 4. Lambda Execution
- **lambda-invoke.png** → CLI invoke command run.  
- **response.json** → Output file showing:
  ```json
  {
    "statusCode": 200,
